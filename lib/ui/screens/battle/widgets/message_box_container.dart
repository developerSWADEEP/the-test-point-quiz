...
class _MessageBoxContainerState extends State<MessageBoxContainer> {
  ...
  late double messageBoxDetailsHeightPercentage =
      widget.quizType == QuizTypes.groupPlay ? .69 : .735;
  late final double messageBoxHeightPercentage = 0.755;
  ...
  Widget build(BuildContext context) {
    return Container(
      margin: EdgeInsets.only(
        top:
            widget.topPadding ??
            (MediaQuery.of(context).padding.top + 7.5 + context.height * 0.01),
      ),
      width: context.width * messageBoxWidthPercentage,
      height: context.height * messageBoxHeightPercentage,
      child: Stack(
        children: [
          Align(
